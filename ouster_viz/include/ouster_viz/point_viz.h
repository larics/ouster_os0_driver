...
static const std::string point_fragment_shader_code =
    R"SHADER(
            ...
            void main() {
                gl_FragColor = texture2D(palette, vec2(vcolor, 1));
            })SHADER";
...
static const std::string image_fragment_shader_code =
    R"SHADER(
            ...
            void main() {
                float r = sqrt(texture2D(image, uv).x);
 ...
 */
struct CloudIds {
    GLuint xyz_id, off_id, range_id, key_id, model_id, proj_view_id, palette_id,
        transformation_id, trans_index_id;
    CloudIds() {}
...
struct CloudSetup {
    ...
    size_t n;
    size_t w;
    std::array<double, 16> extrinsics;
    size_t accumulation;
     ...
     * @param extrinsics_ Extrinsic calibration of sensor, row major
    ...
    CloudSetup(const double* xyz_, const double* off_, size_t n_, size_t w_,
               const std::array<double, 16>& extrinsics_,
               size_t accumulation_ = 1)
        : xyz(xyz_),
          off(off_),
          n(n_),
          w(w_),
   ...
   public:
    ...
    Cloud(T* xyz, T* off, const size_t n, const size_t w,
        ...
        : n(n),
          ...
          xyz_changed(true) {
        std::array<GLuint, 5> vertexbuffers;

        glGenBuffers(5, vertexbuffers.data());
        buffers =
            CloudBuffers{vertexbuffers[0], vertexbuffers[1], vertexbuffers[2],
                         vertexbuffers[3], vertexbuffers[4]};
    ...
    explicit MultiCloud(const CloudSetup& setup) : clouds(setup.accumulation) {
        std::generate(clouds.begin(), clouds.end(), [&setup]() {
            return std::unique_ptr<DoubleBuffer<Cloud>>(new DoubleBuffer<Cloud>(
                setup.xyz, setup.off, setup.n, setup.w, setup.extrinsics));
        });
    }
